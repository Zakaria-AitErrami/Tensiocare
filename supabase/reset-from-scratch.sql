-- Destructive reset for the TensioCare HTA public schema.
-- This deletes public patient/profile/report/alert data, but keeps Supabase Auth users.
-- Before running this file, create this Auth user in Supabase:
--   Email: doctor@clinic.hta
--   Password: demo-clinic

drop table if exists public.alert_events cascade;
drop table if exists public.bp_reports cascade;
...
begin
  ...
  if doctor_user_id is null then
    raise exception 'Create the Supabase Auth user doctor@clinic.hta with password demo-clinic before running this reset.';
  end if;
...
where lower(u.email) = lower('doctor@clinic.hta');
